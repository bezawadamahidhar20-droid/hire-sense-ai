...
export default function ScreenCandidatesForm({ jobId }: { jobId: string }) {
  ...
  return (
    <form action={formAction} className="space-y-3">
      ...
      <label className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 px-6 py-8 text-center transition hover:border-violet-400 hover:bg-violet-50/40">
        ...
        <input type="file" name="files" accept=".pdf,.docx,.txt" multiple className="hidden" />
      </label>
      <button
        type="submit"
        disabled={pending}
